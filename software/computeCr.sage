...
def computeCrN(M,r):
    ...
    if r==0 and m<1:
        Inv = computeCrAll(M,0)
        Extra = [1]
        #print(Inv)
        #Inv = mergevectors(RJF[1],Inv)
    ### Generating when r>1 and mixed case
    if r >0 and m<1:
        PreEx = computeCrN(M,r-1)
        Pre = PreEx[0]
        Extra = Pre[2]
        Inv = Pre
        if Extra ==[]:
